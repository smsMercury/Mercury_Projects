...
'-v1.6
	...
	Dim iNumHBGSREPs As Integer
	Dim kb, ka, k As Integer
...
'                Case MTTGTLISTUPDID: modMTTGTLISTUPD.Process_MTTGTLISTUPD pdblTime, pintTmp_Byte_Count, pabytTmp_Msg_Data
                '-v1.8.10 TE
                '
				'+v1.8.14 SV		SPEAR MESSAGES
				Case MTWDENABLEID: modDASMtwdenable pdblTime, pintTmp_Byte_Count, pabytTmp_Msg_Data
				Case MTWDSIGSTATID: modDASMtwdsigstat pdblTime, pintTmp_Byte_Count, pabytTmp_Msg_Data
				Case MTWDTAREQID: modDASMtwdtareq pdblTime, pintTmp_Byte_Count, pabytTmp_Msg_Data
				Case MTWDTASTATID: modDASMtwdtastat pdblTime, pintTmp_Byte_Count, pabytTmp_Msg_Data
				Case MTWDTATRACKID: modDASMtwdtatrack pdblTime, pintTmp_Byte_Count, pabytTmp_Msg_Data
				'-v1.8.14 SV
...
ERR_HANDLER:
    Select Case Err.Number
        ...
        '
        Case 49:
			MsgBox Err.Description & "  " & pintSwap_Msg_ID
            Resume Next
...
70  pusrDAS_Rec.sOrigin = sGetOriginID(pusrDAS_Rec.lOrigin_ID)
    ...
    ElseIf (lType = 26) Then ' DL_DF_LOB_LIST
		...
		lRecSize = agSwapWords&(pusrVarRecHdr.lRecSize)
		if (lRecSize > LenB(pusrVarRecHdr)) Then
...
90  With pusrMTRUNMODE
		pusrDAS_Rec.lStatus = agSwapBytes%(.iRunMode)
		pusrDAS_Rec.lTag = agSwapBytes%(.iLmbRunMode)
		pusrDAS_Rec.lFlag = agSwapBytes%(.iHbRunMode)
		pusrDAS_Rec.lCommon_ID = agSwapBytes%(.iSprRunMode)
		pusrDAS_Rec.lIFF = agSwapBytes%(.iSsRunMode)
		pusrDAS_Rec.lTarget_ID = agSwapBytes%(.iAxeRunMode)
100     pusrDAS_Rec.sSupplemental = "SYS@" & modRunmodeToString(agSwapBytes%(.iRunMode)) & "," & " LMB@" & modRunmodeToString(agSwapBytes%(.iLmbRunmode)) & "," & " HB@" & modRunmodeToString(agSwapBytes%(.iHbRunmode)) & "," & " SPR@" & modRunmodeToString(agSwapBytes%(.iSprRunmode)) & "," & " SS@" & modRunmodeToString(agSwapBytes%(.iSsRunMode)) & "," & " AXE@" & modRunmodeToString(agSwapBytes%(.iAxeRunmode))
    End With
...
250                 pusrDAS_Rec.dSpeed = agSwapBytes%(.iTrackSpeed)
                ...
                Else
					bDps = False
					If pusrDAS_Rec.sOrigin = "DPS" Then
						If agSwapBytes%(.iNumContributors) > 10 Then
							If MsgBox("Invalid DPS Track, NumContribs: " & agSwapBytes%(.iNumContributors), vbOKOnly) =  vbOK Then
								bDps = True
							End If
						End If
					End If
...
'
'+v1.8.14
' ROUTINE:  modDasMTWDENABLE
' AUTHOR:   Shaun Vogel
' PURPOSE:  Translate the MTWDENABLE message to a data table
' INPUT:    dblTime = the message time stamp
'           intMsg_Length = the message length (in bytes)
'           abytBuffer() = the message contents
' OUTPUT:   None
' NOTES:
Public Sub modDASMtwdenable(dblTime As Double, intMsg_Length As Integer, abytBuffer() As Byte)
	Dim pusrMTWDENABLE As Mtwdenable
	Dim pintStart As Integer
	Dim pintStruct_Length As Integer
	'
	On Error GoTo Hell
	'
	pintStruct_Length = LenB(pusrMTWDENABLE)
	pintStart = LBound(abytBuffer)
	Call CopyMemory(pusrMTWDENABLE, abytBuffer(pintStart), pintStruct_Length)
	
	Call Process_MTWDENABLE(pusrMTWDENABLE)
	Exit Sub
...
Hell:
	...
	'
	' Log the error
	basCCAT.WriteLogEntry "Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, "(@ line #" & plngERL & ")", "") & " in Filtraw2Das.modDasMTWDENABLE"
	'
	' process the error
	Select Case plngErr_Num
		'
		Case Else:
			Select Case MsgBox("Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, " @ line #" & plngERL, "") & vbCr & "While translating MTWDENABLE message", vbAbortRetryIgnore Or vbExclamation Or vbMsgBoxHelpButton, "Error Translating MTWDENABLE", App.HelpFile, basCCAT.IDH_TRANSLATE_MTWDENABLE)
                Case vbAbort:
                    mblnContinue = False
...
'
'+v1.8.14
' ROUTINE:  modDasMTWDSIGSTAT
' AUTHOR:   Shaun Vogel
' PURPOSE:  Translate the MTWDSIGSTAT message to a data table
' INPUT:    dblTime = the message time stamp
'           intMsg_Length = the message length (in bytes)
'           abytBuffer() = the message contents
' OUTPUT:   None
' NOTES:
Public Sub modDASMtwdsigstat(dblTime As Double, intMsg_Length As Integer, abytBuffer() As Byte)
	Dim pusrMTWDSIGSTAT As Mtwdsigstat
	Dim pintStart As Integer
	Dim pintStruct_Length As Integer
	'
	On Error GoTo Hell
	'
	pintStruct_Length = LenB(pusrMtwdsigstat)
	pintStart = LBound(abytBuffer)
	Call CopyMemory(pusrMtwdsigstat, abytBuffer(pintStart), pintStruct_Length)
	
	Call Process_MTWDSIGSTAT(pusrMtwdsigstat)
	Exit Sub
...
Hell:
	...
	'
	' Log the error
	basCCAT.WriteLogEntry "Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, "(@ line #" & plngERL & ")", "") & " in Filtraw2Das.modDasMTWDSIGSTAT"
	'
	' process the error
	Select Case plngErr_Num
		'
		Case Else:
			Select Case MsgBox("Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, " @ line #" & plngERL, "") & vbCr & "While translating MTWDSIGSTAT message", vbAbortRetryIgnore Or vbExclamation Or vbMsgBoxHelpButton, "Error Translating MTWDSIGSTAT", App.HelpFile, basCCAT.IDH_TRANSLATE_MTWDSIGSTAT)
                Case vbAbort:
                    mblnContinue = False
...
'
'+v1.8.14
' ROUTINE:  modDasMTWDTAREQ
' AUTHOR:   Shaun Vogel
' PURPOSE:  Translate the MTWDTAREQ message to a data table
' INPUT:    dblTime = the message time stamp
'           intMsg_Length = the message length (in bytes)
'           abytBuffer() = the message contents
' OUTPUT:   None
' NOTES:
Public Sub modDASMtwdtareq(dblTime As Double, intMsg_Length As Integer, abytBuffer() As Byte)
	Dim pusrMTWDTAREQ As Mtwdtareq
	Dim pintStart As Integer
	Dim pintStruct_Length As Integer
	'
	On Error GoTo Hell
	'
	pintStruct_Length = LenB(pusrMtwdtareq)
	pintStart = LBound(abytBuffer)
	Call CopyMemory(pusrMtwdtareq, abytBuffer(pintStart), pintStruct_Length)
	
	Call Process_MTWDTAREQ(pusrMtwdtareq)
	Exit Sub
...
Hell:
	...
	'
	' Log the error
	basCCAT.WriteLogEntry "Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, "(@ line #" & plngERL & ")", "") & " in Filtraw2Das.modDasMTWDTAREQ"
	'
	' process the error
	Select Case plngErr_Num
		'
		Case Else:
			Select Case MsgBox("Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, " @ line #" & plngERL, "") & vbCr & "While translating MTWDTAREQ message", vbAbortRetryIgnore Or vbExclamation Or vbMsgBoxHelpButton, "Error Translating MTWDTAREQ", App.HelpFile, basCCAT.IDH_TRANSLATE_MTWDTAREQ)
                Case vbAbort:
                    mblnContinue = False
...
'
'+v1.8.14
' ROUTINE:  modDasMTWDTASTAT
' AUTHOR:   Shaun Vogel
' PURPOSE:  Translate the MTWDTASTAT message to a data table
' INPUT:    dblTime = the message time stamp
'           intMsg_Length = the message length (in bytes)
'           abytBuffer() = the message contents
' OUTPUT:   None
' NOTES:
Public Sub modDASMtwdtastat(dblTime As Double, intMsg_Length As Integer, abytBuffer() As Byte)
	Dim pusrMTWDTASTAT As Mtwdtastat
	Dim pintStart As Integer
	Dim pintStruct_Length As Integer
	'
	On Error GoTo Hell
	'
	pintStruct_Length = LenB(pusrMtwdtastat)
	pintStart = LBound(abytBuffer)
	Call CopyMemory(pusrMtwdtastat, abytBuffer(pintStart), pintStruct_Length)
	
	Call Process_MTWDTASTAT(pusrMtwdtastat)
	Exit Sub
...
Hell:
	...
	'
	' Log the error
	basCCAT.WriteLogEntry "Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, "(@ line #" & plngERL & ")", "") & " in Filtraw2Das.modDasMTWDTASTAT"
	'
	' process the error
	Select Case plngErr_Num
		'
		Case Else:
			Select Case MsgBox("Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, " @ line #" & plngERL, "") & vbCr & "While translating MTWDTASTAT message", vbAbortRetryIgnore Or vbExclamation Or vbMsgBoxHelpButton, "Error Translating MTWDTASTAT", App.HelpFile, basCCAT.IDH_TRANSLATE_MTWDTASTAT)
                Case vbAbort:
                    mblnContinue = False
...
'
'+v1.8.14
' ROUTINE:  modDasMTWDTATRACK
' AUTHOR:   Shaun Vogel
' PURPOSE:  Translate the MTWDTATRACK message to a data table
' INPUT:    dblTime = the message time stamp
'           intMsg_Length = the message length (in bytes)
'           abytBuffer() = the message contents
' OUTPUT:   None
' NOTES:
Public Sub modDASMtwdtatrack(dblTime As Double, intMsg_Length As Integer, abytBuffer() As Byte)
	Dim pusrMTWDTATRACK As Mtwdtatrack
	Dim pintStart As Integer
	Dim pintStruct_Length As Integer
	'
	On Error GoTo Hell
	'
	pintStruct_Length = LenB(pusrMtwdtatrack)
	pintStart = LBound(abytBuffer)
	Call CopyMemory(pusrMtwdtatrack, abytBuffer(pintStart), pintStruct_Length)
	
	Call Process_MTWDTATRACK(pusrMtwdtatrack)
	Exit Sub
...
Hell:
	...
	'
	' Log the error
	basCCAT.WriteLogEntry "Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, "(@ line #" & plngERL & ")", "") & " in Filtraw2Das.modDasMTWDTATRACK"
	'
	' process the error
	Select Case plngErr_Num
		'
		Case Else:
			Select Case MsgBox("Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, " @ line #" & plngERL, "") & vbCr & "While translating MTWDTATRACK message", vbAbortRetryIgnore Or vbExclamation Or vbMsgBoxHelpButton, "Error Translating MTWDTATRACK", App.HelpFile, basCCAT.IDH_TRANSLATE_MTWDTATRACK)
                Case vbAbort:
                    mblnContinue = False
...
'
'+v1.8.14
' ROUTINE:  modDasMTRFCONF
' AUTHOR:   Shaun Vogel
' PURPOSE:  Translate the MTRFCONF message to a data table
' INPUT:    dblTime = the message time stamp
'           intMsg_Length = the message length (in bytes)
'           abytBuffer() = the message contents
' OUTPUT:   None
' NOTES:
Public Sub modDASMtrfconf(dblTime As Double, intMsg_Length As Integer, abytBuffer() As Byte)
	Dim pusrMTRFCONF As Mtrfconf
	Dim pintStart As Integer
	Dim pintStruct_Length As Integer
	'
	On Error GoTo Hell
	'
	pintStruct_Length = LenB(pusrMtrfconf)
	pintStart = LBound(abytBuffer)
	Call CopyMemory(pusrMtrfconf, abytBuffer(pintStart), pintStruct_Length)
	
	Call Process_MTRFCONF(pusrMtrfconf)
	Exit Sub
...
Hell:
	...
	'
	' Log the error
	basCCAT.WriteLogEntry "Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, "(@ line #" & plngERL & ")", "") & " in Filtraw2Das.modDasMTRFCONF"
	'
	' process the error
	Select Case plngErr_Num
		'
		Case Else:
			Select Case MsgBox("Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, " @ line #" & plngERL, "") & vbCr & "While translating MTRFCONF message", vbAbortRetryIgnore Or vbExclamation Or vbMsgBoxHelpButton, "Error Translating MTRFCONF", App.HelpFile, basCCAT.IDH_TRANSLATE_MTRFCONF)
                Case vbAbort:
                    mblnContinue = False
...
'
'+v1.8.14
' ROUTINE:  modDasMTSTATISTICS
' AUTHOR:   Shaun Vogel
' PURPOSE:  Translate the MTSTATISTICS message to a data table
' INPUT:    dblTime = the message time stamp
'           intMsg_Length = the message length (in bytes)
'           abytBuffer() = the message contents
' OUTPUT:   None
' NOTES:
Public Sub modDASMtstatistics(dblTime As Double, intMsg_Length As Integer, abytBuffer() As Byte)
	Dim pusrMTSTATISTICS As Mtstatistics
	Dim pintStart As Integer
	Dim pintStruct_Length As Integer
	'
	On Error GoTo Hell
	'
	pintStruct_Length = LenB(pusrMtstatistics)
	pintStart = LBound(abytBuffer)
	Call CopyMemory(pusrMtstatistics, abytBuffer(pintStart), pintStruct_Length)
	
	Call Process_MTSTATISTICS(pusrMtstatistics)
	Exit Sub
...
Hell:
	...
	'
	' Log the error
	basCCAT.WriteLogEntry "Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, "(@ line #" & plngERL & ")", "") & " in Filtraw2Das.modDasMTSTATISTICS"
	'
	' process the error
	Select Case plngErr_Num
		'
		Case Else:
			Select Case MsgBox("Error #" & plngErr_Num & " - " & pstrErr & IIf(plngERL > 0, " @ line #" & plngERL, "") & vbCr & "While translating MTSTATISTICS message", vbAbortRetryIgnore Or vbExclamation Or vbMsgBoxHelpButton, "Error Translating MTSTATISTICS", App.HelpFile, basCCAT.IDH_TRANSLATE_MTSTATISTICS)
                Case vbAbort:
                    mblnContinue = False
...
Public Function LongTOD(uTod As TodTime) As Long
    ...
    '   3) add the long: this value is the number of microseconds
    '   4) divide by 1 million to convert to seconds
    pdblSec = ((CDbl(uTod.bytHighUsecs) * 4294967296#) + modLong2Dbl(agSwapWords&(uTod.lUsecs))) / 1000000#
    '
    ' Add the seconds to the days
    LongTOD = pdblSec
End Function
