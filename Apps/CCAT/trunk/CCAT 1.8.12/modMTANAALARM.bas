Attribute VB_Name = "modMTANAALARM"
Private MSG_NAME As String
Private Const MSG_ID = 2570

Private mrsData As Recordset
'
'
Public Sub Create_MTANAALARM()
    MSG_NAME = guCurrent.sArchive & "_MTANAALARM"
...
'        .Fields.Append .CreateField("PRI", dbDouble)
        .Fields.Append .CreateField("ChannelActive", dbLong)
...
'        .Fields.Append .CreateField("YY", dbDouble)
        .Fields.Append .CreateField("Usage", dbText)
        ...
        .Fields("Usage").AllowZeroLength = True

    End With
    guCurrent.DB.TableDefs.Append ptblNew
End Sub
'
Public Sub Process_MTANAALARM(uSig As DAS_MASTER_RECORD)
...
'        .Fields("PRI") = uSig.dPRI
        .Fields("ChannelActive") = uSig.lStatus
...
'        .Fields("XY") = uSig.dXY
'        .Fields("YY") = uSig.dYY
        .Fields("Usage") = uSig.sSupplemental
        .Update
    End With
End Sub
