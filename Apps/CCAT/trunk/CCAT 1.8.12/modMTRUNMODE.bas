...
Public Sub Create_MTRUNMODE()
    MSG_NAME = guCurrent.sArchive & "_MTRUNMODE"
...
'        .Fields.Append .CreateField("YY", dbDouble)
        .Fields.Append .CreateField("RunMode", dbText)
        
        .Fields("ReportType").AllowZeroLength = True
        .Fields("Origin").AllowZeroLength = True
        .Fields("RunMode").AllowZeroLength = True
    End With
    guCurrent.DB.TableDefs.Append ptblNew
End Sub
'
Public Sub Process_MTRUNMODE(uSig As DAS_MASTER_RECORD)
...
'        .Fields("XY") = uSig.dXY
'        .Fields("YY") = uSig.dYY
        .Fields("RunMode") = uSig.sSupplemental
        .Update
    End With
End Sub
