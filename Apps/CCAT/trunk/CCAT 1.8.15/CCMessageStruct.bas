...
Public Enum MsgIds
    ...
    MTWDBEAMREQID = 182 'SV 1.8.14
    MTWDENABLEID = 185 'SV 1.8.14
    MTWDJMEFFID = 301 'SV 1.8.14
    MTWDSIGSTATID = 196 'SV 1.8.14
    MTWDTABEAMID = 191 'SV 1.8.14
    MTWDTAREQID = 192 'SV 1.8.14
    MTWDTASTATID = 193 'SV 1.8.14
    MTWDTATRACKID = 194 'SV 1.8.14
    MTRFCONFID = 378 'SV 1.8.14
    MTSTATISTICSID = 120 'SV 1.8.14
End Enum
'
' High Band ID
...
Public Type LobPacket                       '  16 bytes Fixed
    ...
    bytOperRequested As Byte                '   1 byte   12 -  12
        bytFixAssoc As Byte                                             '       3 bytes  13 -  15
        iCorrelation As Integer                                 '       2 bytes  16 -  18
        lAltitude As Long                                               '       4 bytes  19 -  22
        uTODTime As TodTime                                             '       8 bytes  23 -  30
End Type 'LobPacket
'
' Channel status
...
Public Type Mtrunmode3_0                        ' 20 bytes fixed
    ...
    iHbRunmode As Integer                       '   2 bytes  18 -  19
    iSprRunmode As Integer                      '   2 bytes  20 -  21
        iSsRunMode As Integer                                           '       2 bytes  22 -  23
        iAxeRunMode As Integer                                          '       2 bytes  24 -  25
    bytPad0(1 To 2) As Byte                     '   2 bytes  26 -  27
...
'-v1.8.15
Public Type Mtwdbeamreq
        uMsgHdr As Msg_Hdr
        bytTaWanted(1 To 16) As Byte
End Type 'Mtwdbeamreq

Public Type Mtwdenable
        uMsgHdr As Msg_Hdr
        bytTaEnable(1 To 16) As Byte
End Type 'Mtwdenable

Public Type Mtwdjmeff
        uMsgHdr As Msg_Hdr
        iIdx As Integer
        iAltitude As Integer
        uPtRec(1 To 5) As LatLon
        uPtLoss(1 To 5) As Integer
End Type 'Mtwdjmeff

Public Type Spear_Sig_Status_Rec
        bytType1StatA(1 To 32) As Byte
        bytType1StatB(1 To 32) As Byte
        bytType1StatM(1 To 16) As Byte
        bytSim1Active As Byte
        bytType1SimChanStat(1 To 32) As Byte
        bytType2StatA(1 To 32) As Byte
        bytType2StatB(1 To 32) As Byte
        bytSim2Active As Byte
        bytType2SimChanStat(1 To 32) As Byte
        bytPad(1 To 2) As Byte
End Type

Public Type Mtwdsigstat
        uMsgHdr As Msg_Hdr
        uSigStatus As Spear_Sig_Status_Rec
End Type 'Mtwdsigstat

Public Type TaT2LocRec
        bytTaSpValid As Byte
        bytPad(1 To 3) As Byte
        lTaSecAlt As Long
        uTaSpLoc As LatLon
End Type

Public Type Mtwdstaloc
        uMsgHdr As Msg_Hdr
        bytTaId As Byte
        bytPad(1 To 3) As Byte
        uT2Loc As TaT2LocRec
End Type 'Mtwdstaloc

Public Type TaLocRec
        lTaAltitude As Long
        uTaLoc As LatLon
        lTaTimeSec As Long
        lTaTimeUs As Long
...
Public Type TaRec
        bytTaId As Byte
        bytTaPriority As Byte
        bytTaRxClass As Byte
        bytTaJamTech As Byte
        lTaFreq As Long
        lTaBandwidth As Long
        iTaPowerCtrl As Integer
        bytPad0(1 To 2) As Byte
        lTaRadius As Long
        lTaAltRate As Long
        lTaVelocity As Long
        iTaDirection As Integer         'Bam16
        iTrackId As Integer
        uTaCommandedLoc As TaLocRec
        iTaDir As Integer                       'Bam16
        bytPad1(1 To 2) As Byte
        lTaMag As Long
        lTaVel As Long
        lTaPa As Long
        iTaPhd As Integer                       'Bam16
        bytPad2(1 To 2) As Byte
        lTaPtoSec As Long
        lTaPtoUs As Long
End Type

Public Type Mtwdtareq
        uMsgHdr As Msg_Hdr
        bytTaAction As Byte
        bytPad(1 To 3) As Byte
        lTaMask As Long
...
Public Type TaStatusRec
        bytTaStatus As Byte
        bytTaTechState As Byte
        bytTaBeamData As Byte
        bytPad As Byte
End Type

Public Type Mtwdtastat
        uMsgHdr As Msg_Hdr
        bytTaId As Byte
        uTaStatusRec As TaStatusRec
        bytTaRspType As Byte
        bytTaAction As Byte
        bytPad(1 To 2) As Byte
End Type 'Mtwdtastat

Public Type TaTrackRec
        bytTaErpMeet As Byte
        bytPad As Byte
        iTaErpReq As Integer
        iTaErpXmit As Integer
        iTaBearing As Integer           'Bam16
        lTaRange As Long
        uTaTrackLoc As TaLocRec
End Type

Public Type Mtwdtatrack
        uMsgHdr As Msg_Hdr
        bytTaId As Byte
        bytPad(1 To 3) As Byte
        uTaTrack As TaTrackRec
End Type

Public Type BlankingConfig
        lBlankingConfig(1 To 14, 1 To 5) As Long
End Type

Public Type PatchPanelConfig
        lPaths(1 To 24) As Long
        lSb1Polarization As Long
        lLbPort1Pwr As Long
        lLbPort3Pwr As Long
        lMbPort1Pwr As Long
        lMbPort2Pwr As Long
...
Public Type AitgConfig
        lAitg1 As Long
        lAitg2 As Long
End Type

Public Type RfTxConf
        uBlanking As BlankingConfig
        uPatchPanel As PatchPanelConfig
        uAitg As AitgConfig
End Type

Public Type Mtrfconf
        uMsgHdr As Msg_Hdr
        uConfig As RfTxConf
        bytMissionChange As Byte
        bytIsDefaultPanel As Byte
        bytIsDefaultBlanking As Byte
        bytPad As Byte
End Type 'Mtrfconf

Public Type BufferStatistics
        lTotalAvailBufs As Long
        lTotalBufsInuse As Long
End Type

Public Type NetworkStatistics
        lMessagesReceived As Long
        lMessagesReceivedError As Long
        lMessagesSent As Long
        lMessagesTransmitError As Long
End Type

Public Type CpuUtilStatistics
        lUtilSinceLastStatRequest As Long
        lUtilSinceLastModeChange As Long
End Type

Public Type StatisticsHdr
        uStatLength As Long
        uKindOfStat As Long
End Type

Public Type ComStatistics
        uStatHdr As StatisticsHdr
        uTod As TodTime
        mode As Integer
        submode As Integer
        uMsgBufStat(1 To 3) As BufferStatistics
        uNetworkStat(1 To 2) As NetworkStatistics
        uCpuUtilStat(1 To 14) As CpuUtilStatistics
End Type 'ComStatistics

Public Type AppStatistics
        uStatHdr As StatisticsHdr
        bytAppStatData(0) As Byte
        bytPad(1 To 3) As Byte
End Type 'AppStatistics

Public Type Mtstatistics
        uMsgHdr As Msg_Hdr
        uComStats As ComStatistics
        uAppStats As AppStatistics
End Type 'Mtstatistics
'-v1.8.15
